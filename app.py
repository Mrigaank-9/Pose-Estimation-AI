...
def main():
    ...
    st.write("Choose a video to detect exercises.")

    # Options for selecting video input
    video_source = st.radio("Select video source:", ("Upload your own video", "Use example video"))
    ...
    # If user chooses to use an example video
    elif video_source == "Use example video":
        # Dropdown to select between different example videos
        example_video = st.selectbox(
            "Choose an example video",
            ("pushup.mp4", "squat.mp4", "jumping_jack.mp4", "pullup.mp4", "russian_twist.mp4")
        )

        # Example video path (adjust according to your file structure)
        example_video_path = os.path.join("Test", example_video)
        temp_video_path = example_video_path

        st.video(example_video_path)

    if temp_video_path and st.button("Process Video"):
    ...
    st.markdown(
        """
        ### Overview
        This AI uses computer vision and machine learning to detect exercises from video files.
        Follow these steps:
        1. Select the video source.
        2. Click the "Process Video" button.
        3. The AI will detect the exercise with the highest repetition and the average confidence level.
